...
``` {r variant-mean-depth, echo = F, warning = F}
...
``` {r var-depth-summary, echo = F, warning = F, include = F}
# var_depth_04
...
``` {r variant-missingness, echo = F, warning = F, include = F}
...
``` {r var-miss-summary, echo = F, warning = F}
# var_miss_04
...
var_freq_04$maf <- var_freq_04 %>% select(a1, a2) %>% apply(1, function(z) min(z))
var_freq_05$maf <- var_freq_05 %>% select(a1, a2) %>% apply(1, function(z) min(z))
var_freq_06$maf <- var_freq_06 %>% select(a1, a2) %>% apply(1, function(z) min(z))
var_freq_07$maf <- var_freq_07 %>% select(a1, a2) %>% apply(1, function(z) min(z))
var_freq_12$maf <- var_freq_12 %>% select(a1, a2) %>% apply(1, function(z) min(z))
...
``` {r, maf-summary, echo = F, warning = F}
...
``` {r, mean-depth-ind, echo = F, warning = F}
# load data
...
``` {r, het-ind, echo = F, warning = F, include = F}
# load data
...
``` {r, bar-04-samples, echo = F, warning = F}
# LOAD THE PCA DATA
...
``` {r, PCA-04-samples, echo = F, warning = F}
# plot pca
f <- pca_04 %>% ggplot(aes((signif(pve_04$pve[1], 3)*PC1), 
  ...
  xlab(paste0("PC1 (", signif(pve_04$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
...
g <- pca_05 %>% ggplot(aes((signif(pve_05$pve[1], 3)*PC1), 
  ...
  geom_label(nudge_y = 2) + scale_color_manual(values = c("red", "blue", "orange")) + coord_equal() + theme_light() + 
  xlab(paste0("PC1 (", signif(pve_05$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
...
h <- pca_06 %>% ggplot(aes((signif(pve_06$pve[1], 3)*PC1), 
  ...
  xlab(paste0("PC1 (", signif(pve_06$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
...
i <- pca_07 %>% ggplot(aes((signif(pve_07$pve[1], 3)*PC1), 
  ...
  xlab(paste0("PC1 (", signif(pve_07$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
...
j <- pca_12 %>% ggplot(aes((signif(pve_12$pve[1], 3)*PC1), 
  ...
  geom_label(nudge_y = 2) + scale_color_manual(values = c("orange", "blue", "red")) + coord_equal() + theme_light() + 
  xlab(paste0("PC1 (", signif(pve_12$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
