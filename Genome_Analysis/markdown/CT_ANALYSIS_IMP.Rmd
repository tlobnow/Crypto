...
Illumina <- Crypto_Detection_21 %>%
  mutate(illumina = Mouse_ID %in% c("AA_0900", "AA_0866", "AA_0942")) %>%
  filter(illumina == T)

map <- Crypto_Detection_21 %>% leaflet() %>% addProviderTiles("CartoDB") %>% setView(lat = 52.520007, lng =13.404954, zoom = 6)
map1 <- map %>%
  ...
  addLegend("bottomleft", 
            ...
            opacity = 1) %>%
  addLayersControl(overlayGroups = c('Illumina', "Samples (total)"),
...
tyzzeri <- isolates_new %>% filter(Species == "C.tyzzeri")
parvum <- isolates_new %>% filter(Species == "C.parvum")

mus <- isolates_new %>% filter(Host == "Mus")
cattle <- isolates_new %>% filter(Host == "Cattle")
human <- isolates_new %>% filter(Host == "Human")

map <- isolates_new %>% leaflet() %>% addProviderTiles("CartoDB") %>% setView(lat = 52.520007, lng =13.404954, zoom = 1)
map1 <- map %>%
  addPolylines(lat = c(55.0000, 53.6000, 53.51885, 52.8875  , 52.6053, 51.8978, 45.0000), 
               lng = c(10.0000, 11.4563, 12.4464,13.8119 , 13.8756, 13.8103, 10.0000), 
               color = "purple", 
               weight = 10, 
               opacity = 0.4) %>%
  addCircleMarkers(data = isolates_new,
                   col = "black",
                   opacity = 1,
                   radius = 4,
                   group = "Background") %>%
  ...
  addLegend("bottomright", 
            colors = c('#FF7F7F', 'blue'),
            labels = c('C.tyzzeri',
                       'C.parvum'),
            title = 'Species',
            opacity = 1) %>%
  addLegend("bottomleft", 
            colors = c('yellow', '#82d682', '#ca80e9'),
            labels = c('Mus',
                       'Cattle',
                       'Human'),
            title = 'Host',
            opacity = 1) %>%
  addLayersControl(overlayGroups = c('Background', 'C.tyzzeri', 'C.parvum', 'Mus', 'Cattle', 'Human'), 
                   options = layersControlOptions(collapsed = T))
map1
```
...
Alternatively, this can be run in parallel after specification of all individuals in a list:
    ...
    for i in /SAN/Ctyzzeri/all/results/filteredReads/*.trimmed.R1.fastq.gz; do echo $(basename -a -s .trimmed.R1.fastq.gz $i); done > inds
...
Usually then, it is best practice to produce one dataset with a good MAF threshold and keep another without any MAF filtering at all.
    ...
    summary(var_freq_38$maf) # Mean = 0.235     --> ?
...
What is more important here is that we set a good maximum depth cufoff. As the outliers show, some regions clearly have extremely high coverage and this likely reflects mapping/assembly errors and also paralogous or repetitive regions. We want to exclude these as they will bias our analyses. Usually a good rule of thumb is something the mean depth x 2 
    ...
    summary(var_depth_38$mean_depth) # Mean = 97.91384  x 2 = 195.83

- MIN_DEPTH = 10
- MAX_DEPTH = 85.64 / 135.01 / 161.03 / 195.83
...
The results indicate relatively high missingness, especially among _C.tyzzeri_ samples. One thing to note here is that vcftools inverts the direction of missigness, so our 10% threshold means we will tolerate 90% missingness. Different datasets will likely have different missingness profiles. Typically missingness of 75-95% is used. We will accept max. 15% missingness due to the C.tyzzeri dataset (=0.85).
    ...
    summary(var_miss_38$fmiss) # Mean = 0.03957     --> MISS=0.90

- MISS = 0.85 / 0.9 / 0.9 / 0.9
...
a <- ggplot(var_qual_05, aes(qual)) + geom_density(fill = "red", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1000)
b <- ggplot(var_freq_05, aes(maf)) + geom_density(fill = "red", colour = "black", alpha = 0.3) + theme_light()
c <- ggplot(var_depth_05, aes(mean_depth)) + geom_density(fill = "red", colour = "black", alpha = 0.3) + theme_light()
d <- ggplot(ind_depth_05, aes(depth)) + geom_histogram(fill = "red", colour = "black", alpha = 0.3) + theme_light() + xlim(0,200)
e <- ggplot(var_miss_05, aes(fmiss)) + geom_density(fill = "red", colour = "black", alpha = 0.3) + theme_light() + xlim(0,0.8)
f <- ggplot(ind_miss_05, aes(fmiss)) + geom_histogram(fill = "red", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1)
...
a <- ggplot(var_qual_07, aes(qual)) +        geom_density(fill = "green", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1000)
b <- ggplot(var_freq_07, aes(maf)) +         geom_density(fill = "green", colour = "black", alpha = 0.3) + theme_light()

c <- ggplot(var_depth_07, aes(mean_depth)) + geom_density(fill = "green", colour = "black", alpha = 0.3) + theme_light() 
d <- ggplot(ind_depth_07, aes(depth)) +      geom_histogram(fill = "green",      colour = "black", alpha = 0.3) + theme_light() + xlim(0,200)

e <- ggplot(var_miss_07, aes(fmiss)) +       geom_density(fill = "green", colour = "black", alpha = 0.3) + theme_light() + xlim(0,0.8) #+ ylim(0,60)
f <- ggplot(ind_miss_07, aes(fmiss)) + geom_histogram(fill = "green",      colour = "black", alpha = 0.3) + theme_light()+ xlim(0,1)
...
a <- ggplot(var_qual_18, aes(qual)) + geom_density(fill = "blue", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1000)
b <- ggplot(var_freq_18, aes(maf)) + geom_density(fill = "blue", colour = "black", alpha = 0.3) + theme_light()+ ylim(0,50)
c <- ggplot(var_depth_18, aes(mean_depth)) + geom_density(fill = "blue", colour = "black", alpha = 0.3) + theme_light()
d <- ggplot(ind_depth_18, aes(depth)) + geom_histogram(fill = "blue",      colour = "black", alpha = 0.3) + theme_light() + xlim(0,200)
e <- ggplot(var_miss_18, aes(fmiss)) + geom_density(fill = "blue", colour = "black", alpha = 0.3) + theme_light() + xlim(0,0.8)
f <- ggplot(ind_miss_18, aes(fmiss)) + geom_histogram(fill = "blue",      colour = "black", alpha = 0.3) + theme_light() + xlim(0,1)
...
# load quality data
var_qual_38 <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.lqual", col.names = c("chr", "pos", "qual"), skip = 1)
# load maf data
var_freq_38 <- read_delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.frq", col_names = c("chr", "pos", "nalleles", "nchr", "a1", "a2"), skip = 1)
var_freq_38$a2 <- as.numeric(var_freq_38$a2)
var_freq_38$maf <- var_freq_38 %>% dplyr::select(a1, a2) %>% apply(1, function(z) min(z))
# load depth data
var_depth_38 <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.ldepth.mean", col.names = c("chr", "pos", "mean_depth", "var_depth"), skip = 1)
ind_depth_38 <- read_delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.idepth", col_names = c("ind", "nsites", "depth"), skip = 1)
# load missingness data
var_miss_38 <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.lmiss", col.names = c("chr", "pos", "nchr", "nfiltered", "nmiss", "fmiss"), skip = 1)
ind_miss_38 <- read_delim("~/Documents/Github/Crypto/Genome_Analysis/products/vcftools/38_samples.imiss", col_names = c("ind", "ndata", "nfiltered", "nmiss", "fmiss"), skip = 1)

a <- ggplot(var_qual_38, aes(qual)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1000) 
b <- ggplot(var_freq_38, aes(maf)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + theme_light()+ ylim(0,50)
c <- ggplot(var_depth_38, aes(mean_depth)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + theme_light()# + ylim(0,0.025)
d <- ggplot(ind_depth_38, aes(depth)) + geom_histogram(fill = "purple",      colour = "black", alpha = 0.3) + theme_light() + xlim(0,200)
e <- ggplot(var_miss_38, aes(fmiss)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + theme_light() + xlim(0,0.8) #+ ylim(0,60)
f <- ggplot(ind_miss_38, aes(fmiss)) + geom_histogram(fill = "purple",      colour = "black", alpha = 0.3) + theme_light() + xlim(0,1)

stats38 <- ggarrange(a,b,c,d,e,f, ncol = 2, nrow = 3, 
                  labels = c('Variant Quality', 'Minor Allele Frequency', 'Variant Mean Depth', 
                             'Mean Depth per individual', 'Variant Missingness', 'Missing data per individual'), 
                  font.label = c(size = 10))

annotate_figure(stats38, top = text_grob("C.tyz + C.par + C.hom (n = 38)", color = "black", face = "bold", size = 14))
```
...
e <- ggplot(var_miss_07, aes(fmiss)) + geom_density(fill = "green", colour = "black", alpha = 0.3) + theme_light() + xlim(0,0.8) +
...
b <- ggplot(var_freq_18, aes(maf)) + geom_density(fill = "blue", colour = "black", alpha = 0.3) + theme_light() 
...
b <- ggplot(var_freq_38, aes(maf)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + theme_light()

c <- ggplot(var_depth_38, aes(mean_depth)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) +
  geom_vline(aes(xintercept = 10), color = "green", size = 1.2) +
  geom_vline(aes(xintercept = 2*97.91384), color = "green", size = 1.2) + 
  theme(legend.position = "none") + theme_light()

d <- ggplot(ind_depth_38, aes(depth)) + geom_histogram(fill = "purple", colour = "black", alpha = 0.3) + theme_light() + xlim(0,200)

e <- ggplot(var_miss_38, aes(fmiss)) + geom_density(fill = "purple", colour = "black", alpha = 0.3) + 
  theme_light() + xlim(0,0.8) +
  geom_vline(aes(xintercept = 0.1), color = "green", size = 1.2) + theme(legend.position = "none")

f <- ggplot(ind_miss_38, aes(fmiss)) + geom_histogram(fill = "purple", colour = "black", alpha = 0.3) + theme_light() + xlim(0,1)

stats38 <- ggarrange(a,b,c,d,e,f, ncol = 2, nrow = 3, labels = c('Variant Quality', 'Minor Allele Frequency', 'Variant Mean Depth', 'Mean Depth per individual', 'Variant Missingness', 'Missing data per individual'),font.label = c(size = 10))
...
``` {r 38_samples-stats, echo = F, warning = F, message = F}
# STATS PCA ###################################################################
pca <- read_table("~/Documents/Github/Crypto/Genome_Analysis/products/pcaFiltered/38_samples_filtered_stats2.eigenvec", col_names = F)
eigenval <- scan("~/Documents/Github/Crypto/Genome_Analysis/products/pcaFiltered/38_samples_filtered_stats2.eigenval")
...
reg[grep("USA_P_OR",  pca$ind)] <- "NAM"
reg[grep("USA_P_AL1", pca$ind)] <- "NAM"
reg[grep("USA_P_AL2", pca$ind)] <- "NAM"
...
reg[grep("USA_P_WI",  pca$ind)] <- "NAM"
reg[grep("USA_P_WA1", pca$ind)] <- "NAM"
reg[grep("USA_P_WA2", pca$ind)] <- "NAM"
reg[grep("COL_H_MD",  pca$ind)] <- "SAM"
reg[grep("USA_H_MO",  pca$ind)] <- "NAM"
reg[grep("USA_H_ID",  pca$ind)] <- "NAM"
...
pca <- as_tibble(data.frame(pca, sample, loc, reg, host, spp, ssp))
...
g <- pve %>% ggplot(aes(PC, pve)) + geom_bar(stat = "identity") + ylab("Percentage variance explained") + theme_light()
g
h <- pca %>% ggplot(aes(signif(pve$pve[1], 3)*PC1, (signif(pve$pve[2], 3))*PC2, 
  col = spp, shape = host, label = sample)) + geom_point(size = 3) + 
  #geom_label(nudge_y = 0.05) +
  ggtitle("PCA C.tyz vs. C.par vs. C.hom (n = 38)") +
  scale_color_manual(values = c("blue", "red", "green", "purple", "yellow", "lightgreen", "darkgreen", "grey", "lightblue", "pink")) + 
  coord_equal() + theme_light() + 
  xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + theme(legend.position = "right")
h
```
...
g <- pve %>% ggplot(aes(PC, pve)) + geom_bar(stat = "identity") + ylab("Percentage variance explained") + theme_light()
g
h <- pca %>% ggplot(aes((signif(pve$pve[2], 3)*PC2), (signif(pve$pve[3], 3))*PC3, col = loc, shape = spp, label = sample)) + geom_point(size = 3) + #geom_label(nudge_y = 4) + 
  ggtitle("PCA C.tyzzeri vs. C.parvum vs. C.hominis (n = 7) [new]") +
...
TP     <- unique(samples18$GeneID)
TPH    <- unique(samples07$GeneID)

df1 <- data.frame(GeneID = TYZ, TYZ)
df2 <- data.frame(GeneID = TP, TP)
df3 <- data.frame(GeneID = TPH, TPH)
...
``` {r, cv-errors, echo = F, message = F, warning = F}
a <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/admixture/05_samples_filtered_stats2.cv.error",header = F, sep = '\t', col.names = "cv_error_05_samples")
b <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/admixture/07_samples_filtered_stats2.cv.error",header = F, sep = '\t', col.names = "cv_error_07_samples")
c <- read.delim("~/Documents/Github/Crypto/Genome_Analysis/products/admixture/18_samples_filtered_stats2.cv.error",header = F, sep = '\t', col.names = "cv_error_18_samples")

cv_error <- bind_cols(a,b,c)
...
The list of populations provided with -l gives the order in which the populations or species shall be plotted. Note, that alternatively, if working with your own data, you could also try [this](https://github.com/ramachandran-lab/pong/blob/master/pong-manual.pdf) for plotting.
...
![Clustering of TYZ internal (05_samples)](~/Documents/Github/Crypto/Genome_Analysis/products/admixture/05_samples_k5.tiff)

#### Clustering of TYZ-PAR-HOM (07_samples)

![Clustering of TYZ-PAR-HOM (07_samples)](~/Documents/Github/Crypto/Genome_Analysis/products/admixture/07_samples_k5.tiff)

#### Clustering TYZ-PAR (18_samples)

![Clustering TYZ-PAR (18_samples)](~/Documents/Github/Crypto/Genome_Analysis/products/admixture/18_samples_k5.tiff)
